...
# getSineAngle() = (upper_ang - lower_ang) / 2 * sin((speed * wave_len) * t + start_ang_offset) + ((upper_ang + lower_ang) / 2)
...
def getSineAngle(t: float, lower_ang: float, upper_ang: float, start_ang: float, speed: float = 100, wave_len: float = 0.000013) -> float:
	...
	return amplitude * math.sin(wave_len*t + offset) + center

# Returns the servo position value of a servo doing a sine wave motion at a given time t
# getSinePos() = (upper_pos - lower_pos) / 2 * sin((speed * wave_len) * t + start_pos_offset) + ((upper_pos + lower_pos) / 2)
# t = current time in the sine wave motion, aka where on the x axis you are in the sine wave
...
def getSinePos(t: float, lower_pos: int, upper_pos: int, start_pos: int, speed: float = 100, wave_len: float = 0.000013) -> int:
	...
	return round(amplitude * math.sin(wave_len*t + offset) + center)
